...
For $iii = 0 To $Num_of_Lines
    For $jjj = UBound($Array_of_Unique_Vars) - 1 To 1 Step -1
        ...
        If $Array_of_Unique_Vars[$jjj] <> $Array_of_Unique_Vars[1] Then
            If $Array_of_Unique_Vars[$jjj] <> "_" And $Array_of_Unique_Vars[$jjj] <> "$'" And StringLower($Array_of_Unique_Vars[$jjj]) <> "$cmdline" And StringLower($Array_of_Unique_Vars[$jjj]) <> "$cmdlineraw" And stringLeft(StringLower($Array_of_Unique_Vars[$jjj]), 6) <> "_crypt" Then
                ...
                EndIf
                $Script_Data = StringReplace($Script_Data, $Array_of_Unique_Vars[$jjj], "$_" &StringTrimLeft( _Crypt_HashData($Array_of_Unique_Vars[$jjj], $CALG_MD4, $CALG_MD4), Random(4,8)))
				$var = StringTrimLeft( _Crypt_HashData($Array_of_Unique_Vars[$jjj], $CALG_MD4), Random(4,8))
				ConsoleWrite($Array_of_Unique_Vars[$jjj] & ":$_" &$var & @CRLF)
            EndIf
        Else
            If $Array_of_Unique_Vars[$jjj] <> "_" And $Array_of_Unique_Vars[$jjj] <> "$'" Then
                ...
                If StringRight($Array_of_Unique_Vars[$jjj], 1) = "&" Then
                    $Array_of_Unique_Vars[$jjj] = StringTrimRight($Array_of_Unique_Vars[$jjj], 1)
                EndIf
                $Script_Data = StringReplace($Script_Data, $Array_of_Unique_Vars[$jjj], "$_" &StringTrimLeft( _Crypt_HashData($Array_of_Unique_Vars[$jjj], $CALG_MD4), Random(4,8)))
                $var = StringTrimLeft( _Crypt_HashData($Array_of_Unique_Vars[$jjj], $CALG_MD4), Random(4,8))
				ConsoleWrite($Array_of_Unique_Vars[$jjj] & ":$_" & $var & @CRLF)
            EndIf
            $vars_finished = True
...
;       If StringLeft($Array_of_Funcs[$jjj], 1) = "_" Then ContinueLoop
        ...
        If $Array_of_Funcs[$jjj] <> $Array_of_Funcs[1] Then
			...
			$func = StringTrimLeft(_Crypt_HashData($Array_of_Funcs[$jjj], $CALG_MD4), Random(4,8))
            $Script_Data = StringReplace($Script_Data, $Array_of_Funcs[$jjj], "_" & $func)
            ConsoleWrite($Array_of_Funcs[$jjj] & ":" & "_" & $func & @CRLF)
        Else
...
Func _Encrypt_Strings($AU3, $file_Simple_Obfuscated)
    ...
    FileWrite($fileHandle, base64("", False))
